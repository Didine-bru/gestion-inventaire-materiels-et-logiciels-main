<div class="mt-20 flex mx-12" x-data="requests">
	<div class="relative shadow-md sm:rounded-lg w-full overflow-scroll" x-data="{...staticData(), ...viewOnly()}">
		<h4 class="ml-4 prose prose-lg font-bold">Liste des Logiciels</h4>
		<div class="p-4 flex flex-row justify-between">
			...
			<?php echo modal_software_form($personnels, $licenses); ?>
			<?php echo modal_delete_warning('
			<p>Voulez-vous vraiment supprimer le logiciel suivant :
				<br />Nom du logiciel : <span x-text="itemToDelete.name"></span>
				<br />Version : <span x-text="itemToDelete.version"></span>
			</p>
			') ?>
			<?php echo modal_personnel_form('personnelFormData', 'personnelModalOpen') ?>
			<?php echo modal_license_form('licenseFormData', 'licenseModalOpen'); ?>
		</div>
		<table class="w-full text-sm text-left text-gray-500">
			<thead class="text-xs text-gray-700 uppercase bg-gray-50">
				<tr>
					<th scope="col" class="px-3 py-3">Nom</th>
					<th scope="col" class="px-3 py-3">Version</th>
					<th scope="col" class="px-3 py-3">Date d'achat</th>
					<th scope="col" class="px-3 py-3">Expiration de la garantie</th>
					<th scope="col" class="px-3 py-3">Licence</th>
					<th scope="col" class="px-3 py-3">Personnels</th>
			...
			<tbody>
				<template x-for="item in data" :key="item.id">
					<tr class="bg-white border-b hover:bg-gray-50">
						...
						<td class="px-3 py-4" x-text="item.version"></td>
						<td class="px-3 py-4" x-text="item.date_purchase"></td>
						<td class="px-3 py-4" x-text="item.date_warranty_expiration"></td>
						<td class="px-3 py-4">
							<template x-if="item.license">
								<a href="#" class="font-medium text-primary hover:underline" @click="licenseToFormData(item.license); licenseModalOpen = true;" x-text="item.license.name"></a>
							</template>
						</td>
						<td class="px-3 py-4">
							<div class="flex flex-col">
								<template x-for="personnel in item.personnels" :key="index">
									<a href="#" class="font-medium text-primary hover:underline" @click="personnelToFormData(personnel); personnelModalOpen = true;" x-text="personnel.name"></a>
								</template>
							</div>
						</td>
						<td class="w-12 p-3">
	...
	L308.238,559.79z M746.627,473.387v402.175H124.438V253.373h402.204l124.407-124.438H0V1000h871.064V348.918L746.627,473.387z" />
		...
		<?php echo pagination(
			"loading ? 'Chargement' : data.length == 0 ? 'Il n\'y a aucun logiciel' : data.length > 1 ? 'Il y a ' + data.length + ' logiciels' : 'Il y a un seul logiciel'"
		) ?>
	</div>
</div>
<script>
	...
	function viewOnly() {
		return {
			...
			licenseFormData: formDataOf('license'),
			licenseToFormData(values) {
				valuesToFormData(this, values, 'license', 'licenseFormData');
			}
		}
	}

	const BASE_URL = '<?php echo base_url() . 'software'; ?>';
	document.addEventListener('alpine:init', () => {
		Alpine.data('requests', () => requestStruct(BASE_URL, 'software'));
	})
</script>
<script>
